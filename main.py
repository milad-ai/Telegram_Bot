...
def is_query_allowed(query: str) -> bool:
    """بررسی می‌کند که آیا کوئری فقط روی جدول test اجرا می‌شود یا خیر"""
    # حذف کامنت‌ها
    query = re.sub(r'--.*$', '', query, flags=re.MULTILINE)
    query = re.sub(r'/\*.*?\*/', '', query, flags=re.DOTALL)
    
    # تبدیل به حروف کوچک برای بررسی آسان‌تر
    query_lower = query.lower().strip()
    
    # بررسی اینکه کوئری با SELECT شروع شود
    if not query_lower.startswith('select'):
        return False
    
    # بررسی وجود کلمات کلیدی ممنوعه
    forbidden_keywords = ['insert', 'update', 'delete', 'drop', 'create', 'alter', 'truncate']
    for keyword in forbidden_keywords:
        if f' {keyword} ' in f' {query_lower} ':
            return False
    
    # بررسی اینکه فقط از جدول test استفاده شود
    # استفاده از regex برای یافتن جدول‌های مورد استفاده
    ...
    if all_tables and not all(table == 'test' for table in all_tables):
        return False
...
def process_classroom_sql(update: Update, context: CallbackContext, sql_text: str):
    ...
    if not is_query_allowed(sql_text):
        update.message.reply_text(
            ...
            "• دستورات INSERT, UPDATE, DELETE, DROP, CREATE, ALTER ممنوع هستند\n"
            "• سایر جدول‌ها غیرقابل دسترسی هستند\n\n"
            "💻 لطفاً یک کوئری SELECT معتبر برای جدول test ارسال کنید:",
            reply_markup=ReplyKeyboardMarkup([["🔙 بازگشت به منو اصلی"]], one_time_keyboard=True, resize_keyboard=True)
        )
        return
    ...
    try:
        with engine.begin() as conn:
            ...
            if rows:
                ...
                # اضافه کردن داده‌ها (حداکثر 10 ردیف)
            ...
            result_message += "💡 می‌توانید کوئری دیگری اجرا کنید یا به منو اصلی بازگردید."
