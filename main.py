    ...
    try:
        with engine.begin() as conn:
            if password is None:
                ...
                result = conn.execute(
                    text("SELECT name, major, pass, email FROM stuid WHERE student_id = :student_id"),
                    {"student_id": student_id}
                ).fetchone()
                if result:
                    return result[0], result[1], result[2], result[3]  # name, major, password, email
                return None, None, None, None
            else:
                # بررسی شماره دانشجویی و رمز عبور
                result = conn.execute(
                    text("SELECT name, major, email FROM stuid WHERE student_id = :student_id AND pass = :password"),
                    {"student_id": student_id, "password": password}
                ).fetchone()
                if result:
                    return result[0], result[1], None, result[2]  # name, major, None, email
                return None, None, None, None
...
def update_email(student_id: str, email: str):
    """ایمیل دانشجو را به‌روزرسانی می‌کند"""
    try:
        with engine.begin() as conn:
            conn.execute(
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if user_state.get(chat_id) == "waiting_student_id":  
        ...
        name, major, _, email = get_student_info(student_id)  
        ...
        name, major, _, email = get_student_info(student_id, password)  
        ...
        if name and major:  
            ...
            email_info = ""
            if email:
                email_info = f"📧 ایمیل ثبت شده: {email}\n"
        ...
        if text == "🚀 تمرین جدید":  
            ...
            current_email = context.user_data.get("email", "")
            email_message = "📧 لطفاً ایمیل خود را وارد کنید:\n\n"
            if current_email:
                email_message += f"📭 ایمیل فعلی: {current_email}\n\n"
            email_message += "📩 این ایمیل برای اطلاع‌رسانی نتایج استفاده خواهد شد."
    ...
    elif user_state.get(chat_id) == "waiting_email":  
        if text == "🔙 بازگشت به منو اصلی":  
            user_state[chat_id] = "completed"  
            update.message.reply_text("🏠 بازگشت به منو اصلی:", reply_markup=get_main_menu())  
            return  
        ...
        student_id = context.user_data["student_id"]  
        if update_email(student_id, email):  
            context.user_data["email"] = email
            user_state[chat_id] = "completed"  
            update.message.reply_text(  
                "✅ ایمیل با موفقیت ثبت شد!\n\n"  
                f"📧 ایمیل شما: {email}\n"  
                "📩 از این پس نتایج به این ایمیل ارسال خواهد شد.\n\n"  
                "🏠 بازگشت به منو اصلی:",  
                reply_markup=get_main_menu()  
            )  
        else:  
