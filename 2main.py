...
def get_student_info(student_id: str):
    ...
    try:
        with engine.begin() as conn:
            ...
            if result:
                return result[0], result[1]  # name, major
            return None, None
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if user_state.get(chat_id) == "waiting_student_id":
        ...
        name, major = get_student_info(student_id)
        ...
        if name and major:
            ...
            update.message.reply_text(
                f"دانشجوی عزیز {name} خوش آمدید! اکنون می‌توانید تمرین را ارسال کنید.\n\n"
                "شماره تمرین را انتخاب کنید:",
                reply_markup=reply_markup
            )
        else:
            update.message.reply_text("شماره دانشجویی یافت نشد. لطفاً شماره دانشجویی صحیح وارد کنید:")
